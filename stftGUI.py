...
from numpy import array, linspace, mean, loadtxt, fft, cumsum, arange, interp,\
        hanning, zeros, concatenate, log10, ndarray, meshgrid
...
class Base:
    ...
    def stft(self, widget):
        ...
        if not isinstance(self.data, ndarray):
            ...
            Txx, Fxx = meshgrid(Time, F)
            fig =  plt.figure()
            ax = fig.gca(projection='3d')
            surf = ax.plot_surface(Txx, Fxx, Sxx.T, rstride=1, cstride=1,
                    linewidth=0, cmap=plt.cm.hot)
            ax.set_xlabel("Time (s)")
            ax.set_ylabel("Frequency (Hz)")
            ax.set_zlabel("PSD (ms^2/Hz)")
